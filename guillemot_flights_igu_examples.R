...
# Get GPS data including meta-data
            points_all <- sqlQuery(gps.db, as.is = TRUE, query=
                       paste("SELECT DISTINCT g.*, t.coldist, t.type, t.flight_in_example
                             FROM guillemots_gps_points_igu AS g, guillemots_gps_points_igu_class AS t
  ...
  points_flight <- points_all[!is.na(points_all$flight_in_example),]
  # Plot quickly to see how this looks
  # plot(points$latitude~points$longitude)
  i <- 1

# par(mfrow = c(1,1))
#   hist(points_flight$elev[points_flight$flight_in_example == 99],
#        breaks = 20)

   pdf("flights_guillemots_IGU.pdf")

  for(i in 1:10){
  points <- points_flight[points_flight$flight_in_example == i,]
...
#   speed_2d <- sqrt((points$veast*points$veast) + (points$vnorth*points$vnorth))
  ...
  point.size.speed <- (0.6 + ((points$elev)/10))
# hist(points_flight$elev)
  # 4. Make map #######
...
ind.t <- TRUE
  points(points$longitude[ind.t], points$latitude[ind.t],
         ...
         pch = 21, cex = 0.8*point.size.speed[ind.t])
